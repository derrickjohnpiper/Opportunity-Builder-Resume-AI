...
export default function CoverLetter() {
  ...
  const fetchSavedDocs = async (uid) => {
    try {
      const res = await fetch(`${API_BASE_URL}/artifacts?user_id=${uid}&artifact_type=cover_letter`);
      const data = await res.json();
  ...
  const handleSaveDocument = async () => {
    if (!saveTitle.trim() || !coverLetterData?.cover_letter) return;
    setSaving(true);
    try {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 2fr) minmax(0, 1fr)', gap: '2rem', alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-muted)' }}>Generate a professional, correctly toned cover letter tailored to the Hiring Manager.</p>
          </div>

          <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', padding: '2rem' }}>
            {baseResume ? (
              <p style={{ color: '#10b981', fontSize: '0.875rem' }}>✓ Base Resume loaded from profile to provide Context.</p>
            ) : (
              <p style={{ color: '#fbbf24', fontSize: '0.875rem' }}>⚠ Warning: No Base Resume found. Output will be generic.</p>
            )}
            <textarea 
            ...
            <input 
              type="text" 
              placeholder="Hiring Manager LinkedIn URL (optional)" 
              value={linkedin}
            ...
            />
            <button className="btn-primary" onClick={handleGenerate} disabled={loading || !jobDescription.trim()}>
              {loading ? "Generating..." : "Generate Cover Letter"}
            </button>
          </div>

          {coverLetterData && coverLetterData.cover_letter && (
            <div className="glass-panel" style={{ padding: '2rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                <h2 style={{ fontSize: '1.25rem', fontWeight: 'bold' }}>Generated Cover Letter</h2>
        ...
        <div className="glass-panel" style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem', borderBottom: '1px solid var(--glass-border)', paddingBottom: '1rem' }}>
            ...
            <h2 style={{ fontSize: '1.1rem', fontWeight: 'bold' }}>Saved Letters</h2>
          </div>
          
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {savedDocs && savedDocs.length === 0 ? (
              <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', textAlign: 'center', padding: '2rem 0' }}>No saved letters yet.</p>
            ) : (
                ...
                <div key={doc.id} style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid var(--glass-border)', borderRadius: '8px', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem' }}>
                    <button onClick={() => {
                        ...
                        a.href = url;
                        a.download = `${doc.title || 'cover_letter'}.txt`;
                        a.click();
                      }} 
